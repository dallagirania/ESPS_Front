...
export class DetailComponent implements OnInit {
  ...
  constructor(
    ...
    private forecastService: PredictionService,
  ) { }
...
getProgressBarColor(date_fin: string): string {
 // Determine and return the color of the progress bar based on the qualification status
  ...
  HistoriquePS(dialog: TemplateRef<any>,id:number) {
    ...
    //     this.router.navigate(["/pages/prediction/"+id]);
    // }

    
  prediction(dialog: TemplateRef<any>,id:number) {
    this.getForecast(id)
    this.dialogservice.open(dialog);
    ...
    this.service.getCCById(id).subscribe(carte => {
      ...
      if(this.carte1.fonction){
        this.service.getMesureCCPredictionResult(id).subscribe(pred => {
          ...
          this.forecastService.getForecastRes(id).subscribe(
            data => {
